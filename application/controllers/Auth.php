...
/**
	...
	public function create_user()
	{
		...
		$level = $this->ion_auth->get_users_groups($user_id)->result();
		$user = $this->user;
		...
		if ($this->form_validation->run() === TRUE && $this->ion_auth->register($identity, $password, $email, $additional_data,$group))
		{
			...
			$this->session->set_flashdata('messageAlert', $this->messageAlert('success', 'Berhasil menambahkan user'));
		...
		else
		{
			...
			$this->data['status'] = [
				...
				'id' => 'status',
				'type' => 'radio',
				'value' => $this->form_validation->set_value('active', isset($value['active'])),
			];
			...
			$this->template->load('template/template','auth/create_user',$this->data);
		}
